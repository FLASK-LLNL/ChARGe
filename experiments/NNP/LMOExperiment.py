...
from charge.servers.molecular_property_utils import get_gap

SYSTEM_PROMPT = """
You are a world-class medicinal chemist with expertise in drug discovery and molecular design. Your task is to propose novel small molecules that are likely to exhibit high binding affinity to a specified biological target, while also being synthetically accessible and having a higher HOMO-LUMO gap.
...
for binding affinity, synthetic accessibility, and HOMO-LUMO gap.
"""
...
calculate its density, synthetic accessibility, and gap. Only return molecules with higher density,
the same or lower synthetic accessibility, and a higher band gap compared to the lead molecule.
...
class LMOExperiment(Experiment):
    def __init__(
        ...
        self.lead_gap = get_gap(lead_molecule)
    ...
    @charge.hypothesis
    def get_gap(self, smiles: str) -> float:
        """
        Calculate the HOMO-LUMO gap (aka band gap), in eV, of a molecule given its SMILES string.
