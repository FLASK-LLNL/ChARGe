...
def monomer_to_repeat_smiles(monomer_smiles: str, strategy: str) -> str:
    ...
    if strategy == "vinyl":
        ...
        if Chem.MolFromSmiles(monomer_smiles).HasSubstructMatch(PATT_LACTAM_R7_EXACT):
            prod = first_valid_product(Chem.MolFromSmiles(monomer_smiles), RXN_LACTAM_R7_TO_N6)
        if prod is None and Chem.MolFromSmiles(monomer_smiles).HasSubstructMatch(PATT_LACTAM_GENERIC):
            prod = first_valid_product(Chem.MolFromSmiles(monomer_smiles), RXN_LACTAM_GENERIC)
...
PATT_THF            = Chem.MolFromSmarts("[O;X2;R]1[C;R][C;R][C;R][C;R]1")   # saturated 5-member cyclic ether
...
PATT_DIOL_1_2_3_    = Chem.MolFromSmarts("[O;H1]-[CX4]-[CX4]-[O;H1]")         # e.g., OCCO (ethylene glycol)
...
PATT_OMEGA_AMINO_ACID_GENERIC = Chem.MolFromSmarts("N-[CH2]-[CH2]-[CH2]-[CH2]-[CH2,$([CH2][CH2])]-C(=O)O")
PATT_LACTAM_R7_HINT     = Chem.MolFromSmarts("O=C1N@@@@@1")  # very permissive “ring amide” hint
...
def suggest_polymerization_rules(monomer_smiles: str) -> List[Suggestion]:
    ...
    if mol.HasSubstructMatch(PATT_EPOXIDE):
        suggestions.append(Suggestion("rop_epoxide", 0.95, "Epoxide ring detected (3-membered cyclic ether) → ROP to poly(alkylene oxide)."))
    if mol.HasSubstructMatch(PATT_LACTAM_ANY_RING):
        suggestions.append(Suggestion("rop_lactam", 0.92, "Lactam ring detected → ROP to polyamide (e.g., Nylon-6)."))

    if mol.HasSubstructMatch(PATT_AB_OMEGA_GENERIC):
        suggestions.append(Suggestion("cond_omega_amino_acid", 0.88, "ω-Amino acid detected → A–B self-condensation to polyamide (nylon-n)."))

    if mol.HasSubstructMatch(PATT_THF):
        suggestions.append(Suggestion("rop_thf", 0.85, "Tetrahydrofuran-like 5-member cyclic ether detected → ROP to poly(tetramethylene oxide)."))
    ...
    if mol.HasSubstructMatch(PATT_ACRYLATE):
        suggestions.append(Suggestion("acrylate", 0.90, "α,β-unsaturated carbonyl (acrylate/amide) → chain-growth at C=C."))

    if mol.HasSubstructMatch(PATT_TERMINAL_VINYL):
        suggestions.append(Suggestion("vinyl", 0.70, "Terminal vinyl group present; generic chain-growth possible (check selectivity)."))

    if mol.HasSubstructMatch(PATT_KETENE):
        suggestions.append(Suggestion("ketene", 0.80, "Ketene C=C=O fragment → conceptual oxyethylene repeat."))

    if mol.HasSubstructMatch(PATT_ALPHA_HA):
        suggestions.append(Suggestion("cond_alpha_hydroxy_acid", 0.75, "α-hydroxy acid pattern → self-condensation polyester (e.g., PLA)."))

    phenolic_matches = mol.GetSubstructMatches(PATT_PHENOL)
    if len(phenolic_matches) == 2:
        suggestions.append(Suggestion("cond_diphenol", 0.70, "Two phenolic –OH groups detected on an aromatic system → poly(arylene ether) by self-condensation."))
...
def choose_strategy_auto(monomer_smiles: str,
    ...
    if not candidates:
        raise ValueError("No suitable single-monomer polymerization rule detected for this monomer.")
    ...
    candidates = [s for s in candidates if s.strategy != "needs_comonomer"] or candidates

    # break ties by specificity
    best_spec = max(SPECIFICITY_ORDER.get(s.strategy, 0) for s in candidates)
    specific = [s for s in candidates if SPECIFICITY_ORDER.get(s.strategy, 0) == best_spec]
    ...
    if len(specific) > 1 and abs(specific[0].confidence - specific[1].confidence) < 0.03:
        ...
        raise ValueError(f"Ambiguous polymerization class candidates: {names}. Please specify a strategy.")

    return winner.strategy, winner.reason

def monomer_to_repeat_auto(monomer_smiles: str,
                           min_confidence: float = 0.80,
                           allow_fallback_to_lower_confidence: bool = True
                           ) -> Tuple[str, str, str]:
    """
...
def run_acrylate_head_to_tail(mol: Chem.Mol) -> Chem.Mol | None:
    ...
    if mol.HasSubstructMatch(PATT_METHACRYLATE):
        ...
        for p in prods:
            try:
                m = Chem.Mol(p[0]); Chem.SanitizeMol(m); return m
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    for s, name in tests:
        try:
            ...
            print(f"{name:22s} -> {wrap_bigsmiles_like(rep):28s}  via {strat:22s} | {why}")
